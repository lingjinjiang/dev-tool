...
function parseYaml(text: string): ParseResult {
  ...
  try {
    ...
    const mark = (err as unknown as { mark?: yaml.Mark }).mark;
    const line = mark ? mark.line + 1 : 1;
    const column = mark ? mark.column + 1 : 1;
    const position = mark ? mark.position : 0;
    const near = mark ? getNearText(trimmed, position) : "";
    return {
      ok: false,
      message: err.message,
      line,
...
function YamlNode({ name, value, defaultExpanded = true }: YamlNodeProps) {
    ...
    return (
      <div className={styles.treeNode}>
        ...
        {expanded && (
          <div className={styles.treeChildren}>
            {entries.map(([key, val]) => (
              <YamlNode key={key} name={key} value={val} />
            ))}
          </div>
        )}
      </div>
...
export default function YamlValidator() {
  ...
  const handleFormat = () => {
    ...
    if (res.ok) {
      const formatted = yaml.dump(res.value);
      setOutput(formatted);
      setInput(formatted);
      setOutputTab("text");
      setResult(res);
  ...
  const handleDownload = () => {
    ...
    downloadText("output.yaml", content);
  };
    ...
    if (outputTab === "tree") {
      return (
        <div className={styles.outputBox}>
          <YamlNode value={result.value} />
        </div>
      );
    }

    return <pre className={styles.outputBox}>{output}</pre>;
  };
  ...
  return (
    <div className={styles.root}>
      <div className={styles.toolbar}>
        ...
        <Tooltip content="转 JSON" relationship="label">
          <Button icon={<BracesRegular />} onClick={handleToJson}>
            YAML → JSON
          </Button>
        </Tooltip>

        <Tooltip content="JSON 转 YAML" relationship="label">
          <Button icon={<ArrowSwapRegular />} onClick={handleFromJson}>
            JSON → YAML
          </Button>
        </Tooltip>
        ...
        <Tooltip content="下载 YAML" relationship="label">
          <Button
      ...
      {result && (
        <>
          {result.ok ? (
            <div className={styles.successBox}>格式正确</div>
          ) : (
